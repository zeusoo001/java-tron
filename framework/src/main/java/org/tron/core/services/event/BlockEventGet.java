...
public class BlockEventGet {
      ...
      for (ContractTrigger trigger : triggers) {
        ...
        if (eventOrLog != null) {
          ...
          if (eventOrLog instanceof ContractEventTrigger) {
            ...
            if ((instance.isContractLogTriggerEnable() && instance.isContractLogTriggerRedundancy()) ||
                (instance.isSolidityLogTriggerEnable() && instance.isSolidityLogTriggerRedundancy())) {
  ...
  private List<ContractTrigger> parseLogs(Protocol.Transaction tx, Protocol.TransactionInfo txInfo) {
    String originAddress = StringUtil.encode58Check(TransactionCapsule.getOwner(tx.getRawData().getContract(0)));
    ...
    for (Protocol.TransactionInfo.Log log : logs) {
      ...
      byte[] contractAddress = TransactionTrace.convertToTronAddress(log.getAddress().toByteArray());
    ...
    for (Protocol.TransactionInfo.Log log : logs) {
      ...
      String strContractAddr =
        ArrayUtils.isEmpty(contractAddress) ? "" : StringUtil.encode58Check(contractAddress);
      SmartContractOuterClass.SmartContract.ABI abi = abiMap.get(strContractAddr);
  ...
  private ContractTrigger processTrigger(ContractTrigger contractTrigger) {
    ...
    if (abi != null && abi.getEntrysCount() > 0 && topics != null && !topics.isEmpty()
      ...
      for (SmartContractOuterClass.SmartContract.ABI.Entry entry : abi.getEntrysList()) {
        if (entry.getType() != SmartContractOuterClass.SmartContract.ABI.Entry.EntryType.Event || entry.getAnonymous()) {
          continue;
        }
