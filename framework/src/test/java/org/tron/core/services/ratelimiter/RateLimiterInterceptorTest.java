...
public class RateLimiterInterceptorTest {
   ...
   * The catch block must release the permit to prevent a permanent semaphore leak
   * (the SimpleForwardingServerCallListener that holds the release logic is never
   * assigned when the exception is thrown).
   */
  @Test
  public void testStartCallExceptionReleasesPermit() throws Exception {
    IPreemptibleRateLimiter perEndpoint = Mockito.mock(IPreemptibleRateLimiter.class);
    when(perEndpoint.tryAcquire(any(RuntimeData.class))).thenReturn(true);
    container.add(KEY_RPC, METHOD_NAME, perEndpoint);
